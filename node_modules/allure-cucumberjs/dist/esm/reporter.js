...
import { ALLURE_RUNTIME_MESSAGE_CONTENT_TYPE, ReporterRuntime, applyLinkTemplate, createDefaultWriter, createStepResult, getEnvironmentLabels, getFrameworkLabel, getHostLabel, getLanguageLabel, getPackageLabel, getThreadLabel, getWorstTestStepResult, md5 } from "allure-js-commons/sdk/reporter";
...
export default class AllureCucumberReporter extends Formatter {
  ...
  onTestCaseStarted(data) {
    ...
    var posixPath = getPosixPathRelativeToProjectRoot(pickle);
    var fullName = "".concat(posixPath, "#").concat(pickle.name);
    var testCaseId = md5("".concat(posixPath, "#").concat((_scenario$name = scenario === null || scenario === void 0 ? void 0 : scenario.name) !== null && _scenario$name !== void 0 ? _scenario$name : pickle.name));
    var result = {
      name: pickle.name,
    ...
    result.labels.push(getLanguageLabel(), getFrameworkLabel("cucumberjs"), getPackageLabel(getPathRelativeToProjectRoot(pickle)), getHostLabel(), getThreadLabel(data.workerId));
  ...
  onTestCaseFinished(data) {
    ...
    this.allureRuntime.updateTest(testUuid, result => {
      var _step$status;
      var step = getWorstTestStepResult(result.steps);
      result.status = (_step$status = step === null || step === void 0 ? void 0 : step.status) !== null && _step$status !== void 0 ? _step$status : Status.PASSED;
